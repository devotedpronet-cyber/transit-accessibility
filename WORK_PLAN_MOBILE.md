...
- [ ] Next step: build the geo-fuzzy-match layer to cross-validate Carris's own bus/tram stops (not just metro) against the wider 2,453-node OSM wheelchair dataset — needed to actually shrink the `unknown` bucket at scale, not just metro. Remaining 26 untagged metro stations stay `unknown` until Willeasy partnership (5.1) or a future OSM tagging pass fills them.
