...
- [x] First step: prototyped Overpass API query (`node["station"="subway"]` in Lisbon bbox, 46 stations found) and diffed `wheelchair=*` tags against `stops.js`'s `ACCESSIBLE_METRO_STATIONS`. Results: OSM confirms 11 stations `wheelchair=yes`, 5 `limited`, 4 `no`, 26 untagged. **6 newly-confirmed stations not in our list** (Aeroporto, Ameixoeira, Chelas, Odivelas, Restauradores, Santa Apolónia) — added to `stops.js`, bringing confirmed total to 21/47. **2 disagreements found**: OSM says `wheelchair=no` for Colégio Militar/Luz and Campo Pequeno, but Metro de Lisboa's own 2026-07-27 announcement lists both accessible — kept as known-accessible (primary source outranks crowdsourced tag) but flagged in `stops.js` comment for re-verification if a rider reports a problem, per the "surface disagreements, don't silently pick one" design in `docs/EXPANSION_ROADMAP.md`. Broader bus/tram stop-level Overpass query (non-metro) returned 2,453 wheelchair-tagged elements Lisbon-wide but almost entirely `platform`/`stop_position` nodes, not directly matchable to Carris's own stop IDs without a name/geo-fuzzy-match layer. (Investigated 2026-09-02.)
- [x] Next step: built the geo-fuzzy-match layer — queried OSM city-wide (bbox covering full Carris Metropolitana area) for `wheelchair=*` on `bus_stop`/`platform`/`tram_stop` nodes (247 found), matched each `wheelchair=yes` node to the nearest Carris stop by haversine distance (≤20m), deduped by Carris stop ID. Result: **131 Carris stop IDs cross-confirmed accessible** — real signal, since Carris's own feed ships `wheelchair_boarding=false` (no info) on all 12,752 stops. Wired into `stops.js` as `OSM_CONFIRMED_STOP_IDS`, applied at fetch time alongside the existing Carris/metro/ferry logic. Combined with 5.2's metro additions, confirmed-accessible stop count goes from 15+9=24 to 21+9+131=161. In-app disclosure note updated in all 7 languages. (Investigated + shipped 2026-09-02.)
- [ ] Remaining gap: 116 of the 247 OSM-tagged nodes didn't match within 20m (name variants, stops OSM tracks that Carris doesn't, or >20m drift) — a looser match (wider radius + name similarity) could recover more but risks false positives; not worth it without a spot-check pass. Remaining unmatched metro stations (26) and full city coverage still depend on Willeasy partnership (5.1) or further OSM tagging.
...
- [x] First step: the public status *page* itself (`metrolisboa.pt/viajar/estado-das-linhas-e-elevadores`) is a static WordPress post (`article:modified_time` was over a month stale) — scraping it would NOT be live. Found the real source instead: the page's own JS calls an undocumented WordPress AJAX endpoint, `GET https://www.metrolisboa.pt/wp-admin/admin-ajax.php?action=estado_linha_ajax_2022_nova_action` — no key, no auth, returns a live HTML fragment with genuine per-elevator status: line → station → `{Equipamento, Nº, Localização (which platform/direction), Estado: "Operacional" | "Fora de serviço"}`, plus a per-line summary (e.g. "4/42 (9,52%) elevadores desta linha estão fora de serviço"). Confirmed real-time: spot-checked shows live individual outages (e.g. Campo Pequeno elevator 1 "Fora de serviço" while elevators 2-3 at the same station are "Operacional" — exactly the per-equipment granularity needed for 5.4). (Investigated 2026-09-02.)
- [ ] Next step (needs a Firebase project — blocked on account access, not a technical dead end): stand up a scheduled Firebase Function that polls this endpoint, parses the HTML fragment (structure is regular: repeating station→equipment blocks, parseable without a headless browser), writes normalized `{station, equipmentId, location, status, lastChecked}` to Firestore. Endpoint is undocumented and unversioned — brittle by nature, monitor for markup changes; keep showing a "last checked" timestamp per the app's never-fabricate-certainty principle.
